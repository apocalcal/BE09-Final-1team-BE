...
public interface NewsRepository extends JpaRepository<News, Long> {
    ...
    // 카테고리별 뉴스 조회
    @Query("SELECT n FROM NewsEntity n WHERE n.categoryName = :category")
    ...
    @Query("SELECT n FROM NewsEntity n ORDER BY n.trusted DESC")
    Page<News> findPopularNews(Pageable pageable);
    
    // 트렌딩 뉴스 조회 (신뢰도 + 생성일 기준)
    @Query("SELECT n FROM NewsEntity n ORDER BY n.trusted DESC, n.createdAt DESC")
    Page<News> findTrendingNews(Pageable pageable);
    
    // 특정 기간 내 뉴스 조회 (페이징)
    @Query("SELECT n FROM NewsEntity n WHERE n.createdAt BETWEEN :startDate AND :endDate")
    Page<News> findByCreatedAtBetween(@Param("startDate") LocalDateTime startDate, 
                                     @Param("endDate") LocalDateTime endDate, 
                                     Pageable pageable);
    
    // 특정 기간 내 뉴스 조회 (List 반환)
    @Query("SELECT n FROM NewsEntity n WHERE n.createdAt BETWEEN :startDate AND :endDate ORDER BY n.createdAt DESC")
    List<News> findByCreatedAtBetween(@Param("startDate") LocalDateTime startDate, 
                                     @Param("endDate") LocalDateTime endDate);
    
    // 신뢰도가 높은 뉴스 조회
    @Query("SELECT n FROM NewsEntity n WHERE n.trusted = true")
...
//    Page<News> findWithSummary(Pageable pageable);
    ...
    // 특정 언론사 뉴스 조회
    @Query("SELECT n FROM NewsEntity n WHERE n.press = :press")
    Page<News> findByPress(@Param("press") String press, Pageable pageable);
    
    // 카테고리별 뉴스 개수 조회
    @Query("SELECT COUNT(n) FROM NewsEntity n WHERE n.categoryName = :category")
    ...
    // oid_aid 리스트로 뉴스 조회
    @Query("SELECT n FROM NewsEntity n WHERE n.oidAid IN :oidAids")
    List<News> findByOidAidIn(@Param("oidAids") List<String> oidAids);
    
    // 같은 생성일, 같은 카테고리, 특정 뉴스 제외
    @Query("SELECT n FROM NewsEntity n WHERE n.createdAt = :createdAt AND n.categoryName = :categoryName AND n.newsId != :excludeNewsId")
    ...
    @Query("SELECT n FROM NewsEntity n WHERE n.oidAid IN :oidAids AND n.newsId != :excludeNewsId")
    List<News> findByOidAidInAndNewsIdNot(@Param("oidAids") List<String> oidAids, 
                                          @Param("excludeNewsId") Long excludeNewsId);
    
    // 특정 기간, 같은 카테고리, 특정 뉴스들 제외
    @Query("SELECT n FROM NewsEntity n WHERE n.createdAt BETWEEN :startDate AND :endDate AND n.categoryName = :categoryName AND n.newsId NOT IN :excludeNewsIds")
