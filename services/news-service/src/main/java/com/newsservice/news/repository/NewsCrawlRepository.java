...
public interface  NewsCrawlRepository extends JpaRepository<NewsCrawl, Long> {
    ...
    // 카테고리별 크롤링 뉴스 조회
    @Query("SELECT nc FROM NewsCrawl nc JOIN nc.category c WHERE c.id = :categoryId")
    Page<NewsCrawl> findByCategoryId(@Param("categoryId") Integer categoryId, Pageable pageable);
